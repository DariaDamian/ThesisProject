...
if (URLHandler.parameters['expert']) {
    //set value of 'display' to null, AKA just show expert without any specified display behavior
    d3.selectAll('.expert').style('display', null); 
    //set value of 'display' to 'none', AKA remove object completely from view
...
window.onload = () => {
    ...
    * */
    ...
    //setup API object (GanterAPI) to call API methods
    const api_prefix = URLHandler.parameters['api'] || '';
    const api = new GanterAPI(api_prefix);

    //select div with id=painting in ganpaint.html
    const painting_sel = d3.select('#painting');
    //In this element, create object GanPaintView with attached eventhandler
