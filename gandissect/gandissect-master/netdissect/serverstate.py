...
class DissectionProject:
    ...
    def get_rankings(self, layer):
        ...
        res = {}
        resCat = {}
        for label in dlayer['labels']:
            res[label['label']] = len(label['units'])
            resCat[label['label']] = label['cat']
        if(len(res) > 5):
            # Filter by harmonic mean
            from statistics import harmonic_mean
            UnitsAverage = harmonic_mean(res.values())
            res = dict(filter(lambda item: item[1] >= UnitsAverage, res.items()))
        else :
            res = dict(sorted(res.items(), key=lambda item: item[1], reverse=True))
        resSet = dict()
        # get only 'pure' names, NO sky-t just sky. Then add -iou
        for na in res:
            if(resCat[na] == 'object'):
                value = 2
            elif(resCat[na] == 'part' ):
                value = 1
            elif(resCat[na] == 'material' ):
                value = 0
            spl = (na.split('-')[0] + "-iou")
            if spl not in resSet:
                resSet[spl] = value
        # only return rankings for those names
        result = []
        for ranking in dlayer['rankings']:
            if ranking['name'] in resSet:
                result.append(dict(name=ranking['name'],
                       metric=ranking.get('metric', None),
                       scores=ranking['score'],
                       category=resSet[ranking['name']]))
        return [resCat,result]
    # Old code
    """ try:
            dlayer = [dl for dl in self.dissect['layers']
                      if dl['layer'] == layer][0]
            ...
            return None
        result = [dict(name=ranking['label'],
                       metric=ranking.get('labelnum', None),
                       scores=ranking['units'])
                  for ranking in dlayer['labels']]
        return result"""
